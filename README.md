...
Refer to [README.md](mapreducers/README.md) in `mapreducers/` to find more info about the mappers and reducers. They can be executed **in shell** or using **hadoop streaming API**. Below are examples for the first mapreduce job. To execute the other one change the digit in the names of the mapper and reducer from 1 to 2.
